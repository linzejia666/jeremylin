...
class Masked_Attention(nn.Module):  #作用是改变Encoder的Attention层，使Attention只关注当前帧，提高注意力机制计算效率（可以不用）
...
class conv_pass(nn.Module):       #这里定义Adapter的结构
...
class ConvPass(nn.Module):      #这里使用的ConPass，后续会替换VIT的Block
