...
class ASTModel(nn.Module):
    def __init__(self, label_dim, input_fdim, input_tdim, unsqueeze=False, fstride=10, tstride=10):
        ...
        self.original_num_patches = self.v.patch_embed.num_patches          #num_patches=576
        self.oringal_hw = int(self.original_num_patches ** 0.5)            #h=w=24   ???
        self.original_embedding_dim = self.v.pos_embed.shape[2]           #embedding_dim=768
        self.mlp_head = nn.Linear(self.original_embedding_dim, label_dim)       #768 -> label_dim
